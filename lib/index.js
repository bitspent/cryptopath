...
class ExtendedKey {
    ...
    toString() {
        ...
        const checkSum = doubleHash(serializedBytes).subarray(0, 4);
...
function deriveChildKey(index, pk, curve) {
    ...
    const il = new BN(ilr.subarray(0, 32));
    const childChainCode = ilr.subarray(32);
    ...
    return {
        ...
        childKey: new ExtendedKey(
            ...
            index,
            childChainCode,
            hash160(pkPublicKeyBytes).subarray(0, 4),
            pk.version
        ),
    };
}
...
function getDerivedPubKey(hexPubKey, hexChainCode, path) {
    ...
    const extKey = new ExtendedKey(pubKey.getPublic(), 0, 0, chainCodeBuf, Buffer.alloc(4), Buffer.alloc(4));
    const { childKey } = deriveChildKeyFromHierarchy(pathBuf, extKey, secp256k1);

    return serializeCompressed(childKey.publicKey.getX(), childKey.publicKey.getY()).toString('hex');
}

function verifyDerivedSignature(hexMessage, hexSigR, hexSigS, numSigV, hexPubKey, hexChainCode, path) {
    const signature = {
        r: hexSigR,
        s: hexSigS,
        recoveryParam: numSigV
    };
    const message = Buffer.from(hexMessage, 'hex');
    const key = ec.recoverPubKey(message, signature, Number(numSigV));
    const recoverPubKey = ec.keyFromPublic(key).getPublic(true, 'hex')
    const derivedPubkey = getDerivedPubKey(hexPubKey, hexChainCode, path);
    const xkey = ec.keyFromPublic(derivedPubkey, 'hex');
    const uncompressed = xkey.getPublic(true, 'hex');
    return {
        valid: uncompressed == recoverPubKey,
        recoveredPubKey: recoverPubKey,
        derivedPubkey: uncompressed
    }
}
